...
class FreeplayState extends MusicBeatState
{
	...
	function reloadWeekShit()
	{
		try {
			...
			NativeAPI.showMessageBox("FreeplayState Error", "An error occurred while loading week data:\n" + Std.string(e));
		}
	}
	...
	function changeSong(by:Int)
	{
		try {
				...
				FlxTween.tween(spr,
					{
						...
						alpha: 1 - (Math.abs(curSong - spr.ID) * 0.5)
			...
			NativeAPI.showMessageBox("FreeplayState Error", "An error occurred while changing the song:\n" + Std.string(e));
		}
	}
	...
	function startSong()
	{
		try {
			...
			else
			{
				...
				var songName = Std.string(songs[curSong][0]);
				// Garante que só o nome simples será usado
			...
			NativeAPI.showMessageBox("FreeplayState Error", "An error occurred while starting the song:\n" + Std.string(e));
		}
	}
	...
	override function update(elapsed:Float)
	{
		try {
			...
			NativeAPI.showMessageBox("FreeplayState Error", "An error occurred during Freeplay update:\n" + Std.string(e));
		}
	}
